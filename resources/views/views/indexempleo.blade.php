...
<body>
    ...
    <section id="central">
    <a  href="http://localhost:8000/crearEmpleos" type="submit" class="btn btn-outline-dark" />Nuevo Empleo</a>
    </br>
    </br>
  ...
  <tbody>
  @foreach($empleo as $row)
    <tr>
      <th scope="row">{{$row['id_empleo']}}</th>
      <td>{{$row['nombre']}}</td>
      <td>{{$row['descripcion']}}</td>
      <td><a href="{{url('/actualizarEmpleos/' .$row->id_empleo.'')}}"  class="btn btn-outline-info">Editar</a></td>
    <td>
    <form method="POST" action="{{url('/eliminarEmpleos/' .$row->id_empleo.'')}}">
    <a href="{{url('/eliminarEmpleos/' .$row->id_empleo.'')}}" class="btn btn-outline-danger"">Eliminar</a>
    <td>
    </form>
    </tr>
    @endforeach
